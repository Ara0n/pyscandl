...
Installing imagemagick (if not already on your system)
======================================================
Starting from version 3.0.0, the lib ``img2pdf`` is replaced by ``wand`` which uses `imagemagick <https://imagemagick.org/>`_ during the generation. You can install imagemagick:

- from your package manager of choice like ``choco``, ``brew``, ``apt`` or ``yay``
- from `their website <https://imagemagick.org/script/download.php>`_
